...
import opentelemetry.instrumentation.aiomysql
from opentelemetry import trace as trace_api
from opentelemetry.instrumentation.aiomysql import AiomysqlInstrumentor, wrappers
from opentelemetry.instrumentation.aiomysql.aiomysql_integration import (
    AiomysqlIntegration,
    _ContextManager,
...
class TestAiomysqlInstrumentor(TestBase):
    def setUp(self):
        super().setUp()
        self.origin_aiomysql_connect = aiomysql.connect
        self.origin_aiomysql_create_pool = aiomysql.create_pool
        aiomysql.connect = mock_connect
        aiomysql.create_pool = mock_create_pool

    def tearDown(self):
        super().tearDown()
        aiomysql.connect = self.origin_aiomysql_connect
        aiomysql.create_pool = self.origin_aiomysql_create_pool
        with self.disable_logging():
            AiomysqlInstrumentor().uninstrument()

    def test_instrumentor_connect(self):
        AiomysqlInstrumentor().instrument()

        cnx = async_call(aiomysql.connect(database="test"))
        ...
        self.assertEqualSpanInstrumentationScope(
            span, opentelemetry.instrumentation.aiomysql
        )
    ...
    def test_instrumentor_connect_ctx_manager(self):
        async def _ctx_manager_connect():
            ...
            async with aiomysql.connect(database="test") as cnx:
    ...
    def test_instrumentor_create_pool(self):
        ...
        pool = async_call(aiomysql.create_pool(database="test"))
    ...
    def test_instrumentor_create_pool_ctx_manager(self):
        async def _ctx_manager_pool():
            ...
            async with aiomysql.create_pool(database="test") as pool:
    ...
    def test_custom_tracer_provider_connect(self):
        ...
        AiomysqlInstrumentor().instrument(tracer_provider=tracer_provider)
    ...
    def test_instrument_connection(self):
        ...
        cnx = AiomysqlInstrumentor().instrument_connection(cnx)
        cursor = async_call(cnx.cursor())
        async_call(cursor.execute(query))
    ...
    def test_no_op_tracer_provider(self):
        cnx = async_call(aiomysql.connect(database="test"))
        AiomysqlInstrumentor().instrument_connection(
    ...
    def test_custom_tracer_provider_instrument_connection(self):
        ...
        cnx = AiomysqlInstrumentor().instrument_connection(
    ...
    def test_uninstrument_connection(self):
        ...
        cnx = AiomysqlInstrumentor().uninstrument_connection(cnx)
        cursor = async_call(cnx.cursor())
        async_call(cursor.execute(query))
...
class TestAiomysqlIntegration(TestBase):
    def setUp(self):
        super().setUp()
    ...
    def test_span_succeeded(self):
        ...
        db_integration = AiomysqlIntegration(
            self.tracer,
            "testcomponent",
            connection_attributes,
    ...
    def test_span_failed(self):
        db_integration = AiomysqlIntegration(self.tracer, "testcomponent")
        mock_connection = async_call(
    ...
    def test_wrap_connect(self):
        aiomysql_mock = AiomysqlMock()
        with mock.patch("aiomysql.connect", aiomysql_mock.connect):
            wrappers.wrap_connect(self.tracer, "-")
            connection = async_call(aiomysql.connect())
            self.assertEqual(aiomysql_mock.connect_call_count, 1)
    ...
    def test_unwrap_connect(self):
        ...
        with mock.patch("aiomysql.connect", aiomysql_mock.connect):
            ...
            connection = async_call(aiomysql.connect())
            self.assertEqual(aiomysql_mock.connect_call_count, 2)
            self.assertIsInstance(connection, mock.Mock)

    def test_wrap_create_pool(self):
        async def check_connection(pool):
            async with pool.acquire() as connection:
                self.assertEqual(aiomysql_mock.create_pool_call_count, 1)
                self.assertIsInstance(
                    connection.__wrapped__, AiomysqlConnectionMock
                )

        aiomysql_mock = AiomysqlMock()
        with mock.patch("aiomysql.create_pool", aiomysql_mock.create_pool):
            wrappers.wrap_create_pool(self.tracer, "-")
            pool = async_call(aiomysql.create_pool())
            async_call(check_connection(pool))

    def test_unwrap_create_pool(self):
        async def check_connection(pool):
            async with pool.acquire() as connection:
                self.assertEqual(aiomysql_mock.create_pool_call_count, 2)
                self.assertIsInstance(connection, AiomysqlConnectionMock)
...
class MockConnection:
    def __init__(self, database, server_port, server_host, user):
        self._conn = MockPymysqlConnection(
