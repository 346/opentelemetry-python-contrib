...
"""
The trace integration with aiomysql based on dbapi integration,
...
.. code-block:: python
    ...
    from opentelemetry.instrumentation.aiomysql import trace_integration

    trace_integration(aiomysql.connection, "_connect", "mysql")
...
def trace_integration(
    ...
    """Integrate with aiomysql library.
...
def wrap_connect(
    ...
    Args:
        name: Name of opentelemetry extension for aiomysql.
        database_system: An identifier for the database management system (DBMS)
            product being used.
        connection_attributes: Attribute names for database, port, host and
            user in Connection object.
        version: Version of opentelemetry extension for aiomysql.
        tracer_provider: The :class:`opentelemetry.trace.TracerProvider` to
            use. If omitted the current configured one is used.
    """
    ...
    try:
        wrapt.wrap_function_wrapper(aiomysql, "connect", wrap_connect_)
    except Exception as ex:  # pylint: disable=broad-except
        logger.warning("Failed to integrate with aiomysql. %s", str(ex))


def unwrap_connect():
    """Disable integration with aiomysql library.
    https://github.com/aio-libs/aiomysql
    """

    unwrap(aiomysql, "connect")
...
def wrap_create_pool(
    ...
    try:
        wrapt.wrap_function_wrapper(aiomysql, "create_pool", wrap_create_pool_)
    except Exception as ex:  # pylint: disable=broad-except
